...
void setup() {
    ...
    if (client.begin("BLESerial")) {
        BLEScanResultsSet *bleDeviceList = client.getScanResults();
        Serial.println("Discover BLE devices...");
        if (client.discoverAsync([](const NimBLEAdvertisedDevice *pDevice) {
            Serial.printf(">>>>>>>>>>> Found a new device: %s\n", pDevice->toString().c_str());
        })) {
            delay(5000);
            Serial.print("Stopping discover...");
            client.discoverAsyncStop();
